...
let rec find_case (x: int) (cmdslst: (int * cmds) list) =
  match cmdslst with
  |[] -> None
  |(n,lst) :: rest -> if n = x then Some(lst) else find_case x rest
...
let rec eval (g : env) (l : env) (st : stack) (log : log) (cmds : cmds) :
  ...
  | IfElse (cmds1, cmds2) :: cmds -> (
      ...
      | _ -> (g, log, None))
  | TryEnd (cmds1) :: cmds -> (
      match eval g l [] log cmds1 with 
      |g, log, Some (v :: _) -> eval g l (v :: st) log cmds
      |g, log, Some [] -> (g, log, None)
      |g, log, None -> eval g l st log cmds)
  | SwitchEnd (cmdslst) :: cmds -> (
      match st with
      | VInt x :: st -> 
        (match find_case x cmdslst with
         |None -> (g, log, None)
         |Some lst -> eval g l st log (lst @ cmds))
      | _ -> (g, log, None))
  | FunEnd (fname, arg, coms) :: cmds -> (
      match fname with
      |Name s -> eval g ((s, Clo(l, fname, arg, coms)) :: l) st log cmds
      |_ ->  (g, log, None))
  | Call :: cmds -> (
      match st with
        Clo (e, fname, arg, coms) :: v :: s_rest -> (
          match arg, fname with 
            Name s1, Name s2 -> 
            (match eval g ((s2,  Clo (e, fname, arg, coms)) :: (s1, v) :: e) [] log coms with
             | (g1, log1, None) -> (g, log, None)
             |(g1, log1, Some []) -> (g, log, None)
             |g1, log1, Some (s::t) -> eval g1 l (s :: s_rest) log1 cmds)
          |_ -> (g, log, None))
      |_ -> (g, log, None))
  | _ -> (g, log, Some st)
...
and switch_parser () =
  ...
  let* cmdslst = many(let* _ = keyword "Case" in
                      let* n = natural in ws >>
                      let* cmds1 = cmds_parser () in
                      pure((n,cmds1))) in
  let* _ = keyword "End" in
  pure(SwitchEnd (cmdslst))
(*make a case parser that makes all of the cases into a list of commands*)

and fun_parser () =
  let* _ = keyword "Fun" in
  let* fname = name_parser () in
  let* arg = name_parser () in 
  let* cmdslst = cmds_parser () in
  let* _ = keyword "End" in
  pure(FunEnd (fname, arg, cmdslst))
...
and cmd_parser () =
  ...
  <|> begin_parser () <|> ifelse_parser () <|> tryend_parser() <|> switch_parser ()
  <|> fun_parser () <|> call_parser ()
